...
class Device(object):
    def __init__(self, base_path, config, fan_max_speed, n_sample_avg, debug = False) -> None:
        ...
        if self.type == "pid":
            ...
            self.controller = FeedForward(float(config["Kp"]), float(config["Ki"]), float(config["Kd"]), float(config["windup_limit"]), float(config["A_setpoint"]), float(config["B_setpoint"]), int(config["T_setpoint"]))
        else:
